...
int main()
{
    ...
    for (int i = 0; i < 47; i++)
    {
        ...
        {
            if (strcmp(names[j], names[j + 1]) > 0) // sorting using bubble sort algorithm
            {
    ...
    for (int i = 0; i < 26; i++) // generating letters A to Z
    {
        arr[i] = 65 + i;
    }
    ...
    for (int i = 0; i < 26; i++)
    {
        for (int j = 0; j < 47; j++)
        {
            // checking the letter from arr[i] to positions of sorted name if condition is true then print the name
            if (arr[i] == names[j][0])
            {
                printf("%s\n", names[j]);
            }
